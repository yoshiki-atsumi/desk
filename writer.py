...
def write_csv(candidates: list[dict], output_dir: Path) -> None:
    ...
    fieldnames = ["候補者名", "政党", "プロフィール", "政策", "その他", "元ファイル", "ページ", "要確認"]
...
def write_markdown(candidates: list[dict], output_dir: Path) -> None:
    ...
    for c in candidates:
        ...
        md = (
            ...
            + header
            + "## プロフィール\n\n"
            + (c.get("profile") or "（情報なし）")
            + "\n\n## 政策・公約\n\n"
            + policies_text
            + ("\n\n## その他\n\n" + other if other else "")
            + "\n"
        )
        (cands_dir / f"{safe}.md").write_text(md, encoding="utf-8")
...
def write_index(candidates: list[dict], output_dir: Path) -> None:
    ...
    for c in candidates:
        ...
        safe = _safe_filename(name)
        review = " [要確認]" if c.get("needs_review") else ""
        lines.append(
            f"## {name}{review}\n"
            f"- **政党**: {party}\n"
            f"- **詳細**: [candidates/{safe}.md](candidates/{safe}.md)\n"
