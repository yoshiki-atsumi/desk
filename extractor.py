...
公平性・正確性のため、候補者欄に書かれているテキストは一字一句漏らさず抽出することが最重要です。
  ...
  "candidates": [
    {
      ...
      "other": "上記フィールドに分類できなかった残りのテキスト全て（連絡先・URL・備考など。なければnull）",
      "raw_text": "候補者欄に書かれている全テキストをそのまま抽出したもの（改行は\\nで表現）",
      "needs_review": false
    }
  ]
}
...
- other: 連絡先・メール・URL・SNS・事務所住所・電話番号・備考など
...
- 縦書きテキストも横書きと同様に処理する
- 候補者の区切りを正確に認識する
- raw_text は候補者欄の全テキストを省略せずそのまま含める
...
def _extract_from_image(
    ...
    for cand in candidates:
        ...
        cand.setdefault("age", None)
        cand.setdefault("catchphrase", None)
        cand.setdefault("profile", "")
        cand.setdefault("policies", [])
        cand.setdefault("other", None)
        cand.setdefault("raw_text", "")
