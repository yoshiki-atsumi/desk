...
{{
  "candidates": [
    {{
      ...
      "party": "政党名または推薦政党（記載なければ null）",
      "profile": "年齢・経歴・学歴・肩書き・キャッチフレーズなど人物に関する情報すべて",
      "policies": [
        "政策・公約の項目をそのまま転記（漏れなく全項目）"
      ],
...
■ 候補者の特定
- 候補者は「人名（姓名）＋写真＋プロフィール＋政策のセット」として掲載されている
- キャッチコピー・スローガン（例: 有言実行、新しい時代へ、市民の声を届けます 等）は候補者名ではない
- 必ず画像で人物の写真・氏名帯を確認して候補者を特定する

■ 重複禁止・全員抽出
- 同一人物は必ず1件のみ登録する。OCRで名前が揺れていても（例: ゆた↔ゆうた）、画像で確認して正しい名前で1件にまとめる
- ページに掲載されている全候補者を漏れなく抽出する

■ 内容の転記
- profile・policies は必ず記載する。空欄・「情報なし」は禁止
- policies は箇条書きの全項目を漏れなく転記する（省略・要約禁止）
- OCRテキストに加えて画像を丁寧に見て、OCRが読み落とした内容も補う
- 書かれている内容をそのまま転記する（解釈・追加禁止）

■ 名前の正確さ
- 候補者名はOCRと画像の両方で一字一字確認する
- 特に長音（ゆ→ゆう、お→おう 等）、濁点、漢字の形を注意して確認する
...
def _dedup_candidates(candidates: list[dict]) -> list[dict]:
    """同一ページ内の重複候補者を除去（名前の類似度で判定）"""
    result = []
    for cand in candidates:
        name = cand.get("name", "")
        # 既存と名前が80%以上類似していたら重複とみなす
        is_dup = any(
            difflib.SequenceMatcher(None, name, r.get("name", "")).ratio() >= 0.8
            for r in result
        )
        if not is_dup:
            result.append(cand)
            ...
            for i, r in enumerate(result):
                if difflib.SequenceMatcher(None, name, r.get("name", "")).ratio() >= 0.8:
                    # policiesが多い方・profileが長い方を採用
                    if len(cand.get("policies", [])) > len(r.get("policies", [])) or \
                       len(cand.get("profile", "")) > len(r.get("profile", "")):
                        result[i] = cand
                    break
    return result
